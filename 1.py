...
def get_live_prices():
    """
    دریافت قیمت از منابع متعدد ایرانی برای اطمینان ۱۰۰٪
    منابع: BRSAPI, Nobitex, Wallex, TadbirPardaz, و غیره
    """
    # لیست توابع دریافت قیمت از منابع مختلف
    
    # --- منبع ۱: BRSAPI (جامع) ---
    try:
        res = requests.get("https://brsapi.ir/FreeTalaGold/api/get_stats", timeout=5).json()
        gold = res['gold'][0]['price']
        usd = res['currency'][0]['price']
        aed = res['currency'][2]['price']
        return (f"💰 **قیمت لحظه‌ای بازار (تومان):**\n━━━━━━━━━━━━━━━\n"
                f"🇺🇸 دلار: {usd:,}\n🇦🇪 درهم: {aed:,}\n⚜️ طلا ۱۸: {gold:,}\n"
                f"━━━━━━━━━━━━━━━\n⏰ {res['date']}\n✅ بانه استور")
    except: pass

    # --- منبع ۲: Nobitex (نرخ تتر/دلار بسیار پایدار) ---
    try:
        res = requests.get("https://api.nobitex.ir/v2/orderbook/USDTIRT", timeout=4).json()
        usd_toman = int(res['lastTradePrice']) / 10
        return (f"💰 **قیمت لحظه‌ای ارز (تومان):**\n━━━━━━━━━━━━━━━\n"
                f"🇺🇸 دلار (تتر): {int(usd_toman):,}\n"
                f"⚠️ نرخ طلا موقتاً در دسترس نیست.\n"
                f"━━━━━━━━━━━━━━━\n✅ منبع: نوبیتکس")
    except: pass

    # --- منبع ۳: Wallex (پشتیبان ارز دیجیتال) ---
    try:
        res = requests.get("https://api.wallex.ir/v1/markets", timeout=4).json()
        usd_val = int(res['result']['symbols']['USDTIRT']['stats']['lastPrice']) / 10
        return (f"💰 **قیمت ارز (تومان):**\n━━━━━━━━━━━━━━━\n"
                f"🇺🇸 دلار: {int(usd_val):,}\n"
                f"━━━━━━━━━━━━━━━\n✅ منبع: والکس")
    except: pass

    # --- منبع ۴: یک API عمومی دیگر (در صورت در دسترس بودن) ---
    try:
        # نرخ ارزهای غیررسمی
        res = requests.get("https://api.iransans.net/v1/currency", timeout=4).json()
        usd = res['data']['usd']['price']
        return f"💰 **قیمت ارز:**\n🇺🇸 دلار: {usd:,} تومان"
    except: pass

    return "⚠️ متأسفانه تمامی وب‌سرویس‌های قیمت‌دهی در این لحظه پاسخگو نیستند. لطفاً ۵ دقیقه دیگر تلاش کنید."
# ================== هندلرها ==================
