...
def get_live_prices():
    try:
        # منبع اول: BRSAPI
        response = requests.get("https://brsapi.ir/FreeTalaGold/api/get_stats", timeout=5)
        ...
        pass # اگر منبع اول خطا داد، برو سراغ بعدی

    try:
        # منبع دوم (Fallback): استعلام دلار از نوبیتکس (بسیار پایدار)
        # قیمت دلار در بازار آزاد معمولاً با قیمت تتر برابر است
        res_tether = requests.get("https://api.nobitex.ir/v2/orderbook/USDTIRT", timeout=5).json()
        usd_price = int(res_tether['lastTradePrice']) / 10 # تبدیل ریال به تومان
        return (f"💰 **قیمت لحظه‌ای ارز (تومان):**\n━━━━━━━━━━━━━━━\n"
                f"🇺🇸 دلار (تتر): {int(usd_price):,}\n"
                f"⚠️ سرویس طلا موقتاً در دسترس نیست.\n"
                f"━━━━━━━━━━━━━━━\n✅ بانه استور")
    except:
        return "⚠️ متأسفانه تمام منابع قیمت‌دهی در حال حاضر قطع هستند. لطفاً دقایقی دیگر امتحان کنید."
